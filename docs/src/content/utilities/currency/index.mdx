...
- **Percentage Formatting**: Format percentages with Indonesian decimal conventions
...
```typescript
import { formatRupiah, formatCompact, formatPercentage, toWords, parseRupiah, splitAmount, percentageOf, difference, validateRupiah } from '@indodev/toolkit/currency';
...
// Format percentage (Indonesian decimal convention)
formatPercentage(0.115);
// '11,5%'

// Format with custom decimals
formatPercentage(0.1152, { decimals: 2 });
// '11,52%'

// Format without symbol
formatPercentage(0.11, { symbol: false });
// '11'

// isPercentage mode (value already represents percentage)
formatPercentage(11.5, { isPercentage: true });
// '11,5%'
...
formatAccounting(-1500000); // '(Rp 1.500.000)'
formatAccounting(-1500, { symbol: false }); // '(1.500)'
```
...
addRupiahSymbol(1500000); // 'Rp 1500000'
```
...
<Callout type="info">
  **Grammar Note:** The function automatically handles Indonesian grammar rules. Numbers are rounded to one decimal place, and trailing
  zeros are removed (e.g., "1 juta" instead of "1,0 juta").
</Callout>
...
Inverse of `formatCompact`. Parses an Indonesian compact amount string into a number.

**Type Signature:**

```typescript
function parseCompact(input: string): number;
```
...
| Name    | Type     | Description                       |
| ------- | -------- | --------------------------------- |
| `input` | `string` | The compact amount string to parse |

**Returns:**

`number` - The parsed value, or `NaN` if the input cannot be unambiguously parsed.

**Recognised Multiplier Shorthands (case-insensitive):**
...
Comma (`,`) is the decimal separator; dot (`.`) is the thousands separator. Both `miliar` and `milyar` (Indonesian spellings of "billion") are accepted.

A multiplier MUST be present — bare numeric input like `"1.5"` or `"1500"` returns `NaN` because compact shorthand = number + multiplier.
...
parseCompact('500k');       // 500000
parseCompact('2rb');        // 2000
parseCompact('1M');         // 1000000000
parseCompact('1,5JT');      // 1500000 (case-insensitive)
parseCompact('  2jt  ');    // 2000000 (whitespace-tolerant)
parseCompact('1,5jt + pajak'); // NaN (garbage text)

// Bare numeric without multiplier → NaN
parseCompact('1.5');        // NaN
parseCompact('1500');       // NaN
```

<Callout type="info">
`parseCompact` reads user-typed shorthand. The reverse direction
(`formatCompact`) emits the long-form words (`"Rp 1,5 juta"`), so a
perfect string round-trip is not expected — but the parsed number
re-formats to the same value.
</Callout>
...
Formats a number as an Indonesian percentage string. Uses comma as decimal separator by default.

**Type Signature:**

```typescript
function formatPercentage(value: number, options?: PercentageOptions): string;
```
...
| Name     | Type               | Description                        |
| -------- | ------------------ | ---------------------------------- |
| `value`  | `number`           | The decimal value (0-1) to format  |
| `options`| `PercentageOptions`| Optional formatting configuration  |
...
interface PercentageOptions {
  ...
  symbol?: boolean;          // Include '%' symbol (default: true)
  decimalSeparator?: string; // Decimal separator (default: ',')
  isPercentage?: boolean;    // Value already represents percentage (default: false)
}
```
...
formatPercentage(0.115, { decimalSeparator: '.' });
// '11.5%'
...
```typescript
// Tax rate display
function TaxRate({ rate }: { rate: number }) {
  return <span>{formatPercentage(rate)} PPN</span>
}

// Discount display
function DiscountBadge({ discount }: { discount: number }) {
  return <span className="discount">{formatPercentage(discount)} OFF</span>
}
...
function ProgressBar({ progress }: { progress: number }) {
  return (
    <div>
      <div style={{ width: formatPercentage(progress, { symbol: false }) }}>
...
// Sales commission
function Commission({ sales, rate }: { sales: number, rate: number }) {
  const commission = sales * rate
  return (
    <div>
      Rate: {formatPercentage(rate)}
      Commission: {formatRupiah(commission)}
    </div>
  )
}
```
...
<Callout type="info">
  **Indonesian Convention:** Uses comma (`,`) as decimal separator, following Indonesian business standards.
</Callout>
...
<Callout type="info">
  **Smart Parsing:** The function automatically detects the format (Indonesian vs International) based on separator positions and handles
  both correctly.
</Callout>
...
<Callout type="warning">
  **Note:** The function converts the integer part only. Decimal values are floored before conversion. For financial documents, format
  decimals separately or round to whole numbers first.
</Callout>
...
calculateTax(-1000000, 0.11);// -110000 (tax credit)
```
...
percentageOf(-200000, -1000000);// 20 (signs cancel)
```
...
- `decimals`: `1` - Shows one decimal place (e.g., "11,5%")
- `symbol`: `true` - Includes '%' symbol
- `decimalSeparator`: `','` - Indonesian standard
- `isPercentage`: `false` - Treats value as decimal (0.115 = 11.5%)
...
<Callout type="info">The library is based on Bahasa Indonesia formal writing standards and common business practices in Indonesia.</Callout>
