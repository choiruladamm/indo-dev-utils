...
- **Text Masking**: Privacy-compliant data display with `all`, `middle`, and `email` masking patterns.
- **Case Converters**: Lightweight `toCamelCase`, `toPascalCase`, and `toSnakeCase` to replace Lodash dependency.
- **Syllable Counting**: Algorithm-based syllable counter for Indonesian text with dipthong awareness.
...
toFormal('gpp bsk aja'); // 'tidak apa-apa besok saja'
```
...
```typescript
function isAlay(text: string): boolean;
```
...
```typescript
function maskText(text: string, options?: MaskOptions): string;
```
...
interface MaskOptions {
  ...
  separator?: string;       // default: none (added in v0.7.0)
...
<Callout type="warning">
  **Breaking Change (v0.7.0):** Options renamed to `visibleStart`, `visibleEnd`, `maskChar`. Added `separator` option. Old names still work with deprecation warnings. Will be removed in v1.0.0.
</Callout>
...
// With separator
maskText('08123456789', { pattern: 'middle', visibleStart: 3, visibleEnd: 4, separator: '-' });
// '081-****-6789'
```
...
countSyllables('');             // 0
countSyllables('a');            // 1
countSyllables('rhythm');       // 1 (no vowels, minimum 1)
```
...
```typescript
function profanityFilter(text: string, options?: FilterOptions): string;
```
...
```typescript
function removeStopwords(text: string, language?: 'id' | 'en' | 'both'): string;
```
