...
// Get age and formatted birth date
const age = getAge('3201018901310123'); // e.g., 35
const birthDateStr = formatBirthDate('3201018901310123'); // '31 Januari 1989'
...
Calculates age from a NIK as of a given reference date (defaults to today).

**Type Signature:**

```typescript
function getAge(nik: string, referenceDate?: Date): number | null;
```
...
| --------------- | -------- | ------------------------------------------- |
| `nik`           | `string` | The 16-digit NIK string                     |
| `referenceDate` | `Date`   | Optional reference date for age calculation |

**Returns:**

`number | null` - Calculated age in years, or `null` if the NIK is invalid.

**Examples:**

```typescript
const age = getAge('3201018901310123');
console.log(age); // e.g., 35 (if today is 2024)
```
